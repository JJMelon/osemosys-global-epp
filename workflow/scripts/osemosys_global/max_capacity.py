...
def main():
    ...
    df_reslimit_units = df_reslimit.loc[
        (df_reslimit["child_object"].str.contains("|".join(dict_reslimit.keys())))
        & (df_reslimit["property"] == "Max Units Built")
        & (df_reslimit["scenario"].str.contains("Base"))
        & (df_reslimit["child_class"] == "Generator")
    ].set_index("child_object")

    df_reslimit_capacity = df_reslimit.loc[
        (df_reslimit["child_object"].str.contains("|".join(dict_reslimit.keys())))
    ...
    if custom_nodes:
        df_re_potentials_custom = pd.read_csv(
            os.path.join(custom_nodes_dir, "RE_potentials.csv")
        )
        df_re_potentials_custom["TECHNOLOGY"] = (
            "PWR"
        ...
        re_potentials_dict = dict(
            zip(
                list(df_re_potentials_custom["TECHNOLOGY"]),
    ...
    for tech, maxad in cap_addition_limit.items():
        ...
        for year in years:
            out_data.append([region, tech, year, max_capacity])
